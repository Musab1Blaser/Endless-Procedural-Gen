...
class Perlin
{
    private:
        ...
        std::vector<int> gen_octave(int octave, int start_x, int y, long long amp, long long wl, long long num_blocks);

    public:
        Perlin(int octaves, long long NUM_CHUNKS, long long CHUNK_SIZE);
        std::vector<int> proc_gen_heights(const int CHUNK_SIZE, const int FIRST_CHUNK, const int NUM_CHUNKS); 
};
